...
With CVRC you can check the status of your friends, watch their profiles, see in what world they are and ask them to send you an invite or send them one yourself. You can create an instance before even starting VRChat, manage your friends, remove or add new ones. You can search for worlds and groups, change your avatar before even starting VRChat, and set up a Media Library so all of your VRChat images are in one place. Send images directly to Discord using the Media Relay webhook system. See actual instance information such as which players are currently in your instance. Start VRChat directly through CVRC and configure other apps to launch alongside it like SlimeVR, VRCVideoCacher, and VRCFaceTracking.

CVRC is a personal fork of [VRCNext](https://github.com/shinyflvre/VRCNext) by shinyflvre, kept in sync with upstream while layering on its own tools and tweaks — a multi-account VRChat launcher, an in-app Avatar Database browser, OSC avatar emotes/height controls, background music, and more.

**About Linux Support**
As of version 2026.42.X AppImage support has been added upstream. It is currently in beta, expect bugs!
...
No. CVRC follows VRChat's Community Guidelines and Terms of Service.
...
Yes. You can use CVRC without logging into your VRChat account.
...
No. CVRC does not store any data and does not operate any external servers.

When you log into your VRChat account you communicate directly with VRChat's API.
There are no servers between CVRC and VRChat. No storage, no data collection, and full privacy.
...
No. CVRC exists in a small grey area related to login.

VRChat states that applications should not request login for API access, although VRCX does this as well.
To respect this guideline, login in CVRC is completely optional and most features can be used without it.
...
Windows sometimes flags applications that are not digitally signed. CVRC is currently not signed.
...
CVRC also includes internal rate limiting to prevent this from happening.

</details>

CVRC is not a replacement for VRCX and never will be. It is a hobby project for a good-looking and useful VRChat launcher with practical features. More features may be added in the future!
...
Yes, you heard it right. CVRC has a messenger, and the best part is that it uses the VRChat API. More specifically, it uses the InviteMessage and ResponseMessage system of the API.
You can send simple messages such as: "Hey Shiny! Do you want to play VRChat later?"
Keep in mind that VRChat rate-limits message changes. Do not spam messages, as you will be rate limited by VRChat. As an additional safety measure, CVRC also applies its own rate limit.
...
* Create group instances directly from CVRC
...
* Browse the in-app Avatar Database tool — downloadable avatars, sortable by name, size, or upload date
...
* Trigger avatar emotes (VRCEmote) and adjust your avatar height live via OSC
...
* Show the song you are listening to on Spotify or iTunes, or pin a specific media source
...
* Show VRChat's native "user is typing..." indicator without putting text in the chatbox
...
* Launch up to 10 fully independent, concurrently-running VRChat instances, each on its own local login session via VRChat's own `--profile=N` launch option
* Rename each slot so you always know which account is which
* Log into a different account in each slot the first time, then just click to relaunch it
...
* Install and update VRCVideoCacher directly from CVRC
...
* Start VRChat directly from CVRC
...
* Play background music while the app is open, with your own custom track or the built-in default
...
CVRC is a fork of VRCNext and is **source-available** under the same custom **VRCNext Non-Commercial Source-Available License (VNCOL)**.
...
Commercialization of VRCNext, forks (including CVRC), or derived versions is prohibited unless explicit permission is granted under the license terms.
...
CVRC is a fork of [VRCNext](https://github.com/shinyflvre/VRCNext) by shinyflvre, kept up to date with upstream releases and extended with its own tools. All credit for the original application, its design, and the bulk of its functionality belongs to shinyflvre and the VRCNext contributors.

AI Disclaimer (carried over from upstream): the frontend (the visual interface you see) was partially created with AI assistance. AI helped with parts of the frontend, mainly CSS and JavaScript such as styling and animations. It did not write the whole frontend. The communication between the frontend and backend, as well as the backend itself, was written entirely by a human.

> [!WARNING]
> **CVRC requires you to log in to VRChat in order to function.**
> CVRC does **not** store any sensitive data and its source code is publicly available, so you can verify this yourself in the source code. You can also build the app manually if you do not trust prebuilt `.exe` files.
>
> According to VRChat:
>
...
>
> This means VRChat asks developers to avoid this wherever possible. However, at this time, applications such as **VRCX** and **CVRC** require login access in order to function.
>
> By using this application, you acknowledge that **VRChat is not responsible** for any issues related to your account that may result from the use of third-party tools like **VRCX** or **CVRC**.
>
...
> `CVRC -> VRChat REST API / WebSocket`
>
...
> In short nothing is saved on any server. Cookies are saved LOCAL on the user machine.
