...
public class AuthController
{
...
#endif
        _ = VrcTryResumeAsync();
        // Auto-popup on version change disabled — this is a customized fork; the changelog modal
        // still exists and can be opened manually (Help menu / "/changelog" console command), it just
        // no longer appears unprompted on launch.
        _ = Task.Run(async () =>
        {
    ...
    private void ApplySettings(JToken data)
    {
        try
        {
            _core.Settings.BotName = data["botName"]?.ToString() ?? "CVRC";
