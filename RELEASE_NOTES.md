...
* Added recognized CVRC name badges — a glowing badge plus an animated name-color wave shown wherever that person's name appears (sidebar, instance list, previews, profile modals). The list of who has one lives in a small public file the app checks on launch, so new badges show up without needing an update
* Avatar Database now has a "My Gofile Folder" field — save a link to your own Gofile folder for one-click access, separate from the community database
* Avatar Logger's history now records which of your VRChat accounts was logged in when each avatar was seen, and that's searchable alongside the avatar name and who wore it
